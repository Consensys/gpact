...
import static tech.pegasys.poc.witnesscodeanalysis.trie.ethereum.trie.CompactEncoding.bytesToPathBinary;
 ...
 */
public class BinaryMerklePatriciaTrie<K extends Bytes, V> implements MerklePatriciaTrie<K, V> {
  ...
  private final BinaryNodeFactory<V> nodeFactory;
   ...
   */
  public BinaryMerklePatriciaTrie(final Function<V, Bytes> valueSerializer) {
    this.nodeFactory = new BinaryNodeFactory<>(valueSerializer);
  ...
  public Optional<V> get(final K key) {
    checkNotNull(key);
    return root.accept(getVisitor, bytesToPathBinary(key)).getValue();
  }
  ...
  public Proof<V> getValueWithProof(final K key) {
    ...
    final Optional<V> value = root.accept(proofVisitor, bytesToPathBinary(key)).getValue();
  ...
  public void put(final K key, final V value) {
    checkNotNull(key);
    checkNotNull(value);
    this.root = root.accept(new PutVisitor<>(nodeFactory, value), bytesToPathBinary(key));
  }

  @Override
  public void remove(final K key) {
    checkNotNull(key);
    this.root = root.accept(removeVisitor, bytesToPathBinary(key));
  }
