...
import (
	...
	"github.com/consensys/gpact/service/relayer/internal/contracts/functioncall"
	"github.com/consensys/gpact/service/relayer/internal/logging"
	"github.com/consensys/gpact/service/relayer/internal/sdk/chainap"
	"github.com/consensys/gpact/service/relayer/internal/sdk/msgstore"
...
func (exec *ExecutorImplV1) start(root *treenode.TreeNode, transID *big.Int) (*functioncall.GpactStart, []byte, error) {
	...
	for i := 0; i < 5; i++ {
		...
		logging.Info("Error submitting transaction, Retry...: %v", err.Error())
...
func (exec *ExecutorImplV1) signal(transID *big.Int, rootChainID *big.Int, rootEvent *functioncall.GpactRoot, rootEventSig []byte, lockedSegments map[string][]*functioncall.GpactSegment, lockedSegmentsSigs map[string][][]byte) error {
	for segChainIDStr, segments := range lockedSegments {
		...
		for i := 0; i < 5; i++ {
			...
			if err == nil {
				logging.Info("Signalling transaction submitted with hash: %v", tx.Hash().String())
				break
			}
			logging.Info("Error submitting transaction, Retry...: %v", err.Error())
			exec.transactOpts.GasPrice, err = chainAP.SuggestGasPrice(context.Background())
