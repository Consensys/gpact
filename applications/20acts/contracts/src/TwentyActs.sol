...
contract TwentyActs is Pausable, AccessControl, CbcDecVer, ResponseProcessUtil {
     ...
     */
    function setInfrastructureAccount(address _account) external {
        require(
     ...
     */
    function setBanned(address _user, bool _banned) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "20ACTS:Must have ADMIN role"
        );
     ...
     */
    function setFrozen(address _user, bool _frozen) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "20ACTS:Must have ADMIN role"
        );
    ...
    function requestWithdrawal(address _erc20, uint256 _amount) external {
        require(
            withdrawalsTime[msg.sender][_erc20] == 0,
        ...
        withdrawalsTime[msg.sender][_erc20] =
    ...
    function finalizeWithdrawal(address _erc20) external {
        ...
        require(
            withdrawalsTime[msg.sender][_erc20] > block.timestamp,
    ...
    function prepareOnTarget(TxInfo calldata _txInfo) external {
        ...
        address source20ActsContract = remoteCrosschainControlContracts[
            _txInfo.sourceBcId
        ];
        require(
        ...
        address sourceErc20Address = erc20AddressMapping[
            _txInfo.targetErc20Address
        ][_txInfo.sourceBcId];
        require(
    ...
    function prepareOnSource(
        ...
        address target20ActsContract = remoteCrosschainControlContracts[
            _txInfo.targetBcId
        ];
        ...
        {
            address targetErc20Address = erc20AddressMapping[
                sourceErc20Address
            ][_txInfo.targetBcId];
    ...
    function finalizeOnTarget(
        ...
        address source20ActsAddress = remoteCrosschainControlContracts[
            _txInfo.sourceBcId
        ];
...
Update reputations of User and the Liquidity Provider (identified by the Liquidity Provider ID) on target chain.
    ...
    */
    function finalizeOnSource(
        ...
        address target20ActsAddress = remoteCrosschainControlContracts[
            _txInfo.targetBcId
        ];
