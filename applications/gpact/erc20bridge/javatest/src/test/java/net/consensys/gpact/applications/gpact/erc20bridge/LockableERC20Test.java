...
public class LockableERC20Test extends AbstractERC20Test {
  ...
  /**
   * Check operation of the contract prior to any data being added to the contract. That is, check
   * the default configuration immediately after deployment.
   */
  @Test
  public void checkDeployment() throws Exception {
    ...
    assert (this.lockableERC20.balanceOfProvisional(this.owner).send().compareTo(INITIAL_SUPPLY_BIG)
        == 0);
    assert (this.lockableERC20
            .allowance(this.owner, this.otherAccount)
            .send()
            .compareTo(BigInteger.ZERO)
        == 0);

    assert (this.lockableERC20.balanceOf(this.otherAccount).send().compareTo(BigInteger.ZERO) == 0);
    assert (this.lockableERC20.balanceOfMin(this.otherAccount).send().compareTo(BigInteger.ZERO)
        == 0);
    assert (this.lockableERC20
            .balanceOfProvisional(this.otherAccount)
            .send()
            .compareTo(BigInteger.ZERO)
        == 0);

    assert (this.lockableERC20
            .accountPallelizationFactor()
            .send()
            .compareTo(BigInteger.valueOf(DEFAULT_ACCOUNT_PARALLELIZATION_FACTOR))
        == 0);
    assert (this.lockableERC20
            .erc20PallelizationFactor()
            .send()
            .compareTo(BigInteger.valueOf(DEFAULT_ERC20_PARALLELIZATION_FACTOR))
        == 0);
  }
  ...
  public void oneTransferNoLocking() throws Exception {
    ...
    assert (this.lockableERC20.balanceOfProvisional(this.otherAccount).send().compareTo(amount)
        == 0);
  }
  ...
  public void multipleParallelTransfersLocking() throws Exception {
    ...
    assert (this.lockableERC20.balanceOf(this.otherAccount).send().compareTo(BigInteger.ZERO) == 0);
    assert (this.lockableERC20.balanceOf(this.otherAccount).send().compareTo(BigInteger.ZERO) == 0);
    assert (this.lockableERC20.balanceOfProvisional(this.otherAccount).send().compareTo(total)
        == 0);
  ...
  public void approveLockingAndNoLocking() throws Exception {
    ...
    assert (this.lockableERC20.allowance(this.owner, this.otherAccount).send().compareTo(amount1)
        == 0);
    this.lockableERC20.approve(this.otherAccount, amount2).send();
    assert (this.lockableERC20.allowance(this.owner, this.otherAccount).send().compareTo(amount2)
        == 0);
  ...
  public void incDecAllowanceNotLocked() throws Exception {
    ...
    assert (this.lockableERC20
            .allowance(this.owner, this.otherAccount)
            .send()
            .compareTo(amount2.subtract(amount1))
        == 0);
    try {
  ...
  public void incDecAllowanceLocked() throws Exception {
    ...
    try {
      ...
      throw new Error(
          "Unexpectedly no revert when increaseAllowance called in excess of parallelization factor");
    } catch (TransactionException ex) {
      System.err.println(
          " Revert Reason: "
              + RevertReason.decodeRevertReason(
    ...
    assert (this.lockableERC20
        ...
        == 0);
    assert (this.lockableERC20
            .allowanceProvisional(this.owner, this.otherAccount)
            .send()
            .compareTo(totalAdd)
        == 0);
    assert (this.lockableERC20
            .allowanceMax(this.owner, this.otherAccount)
            .send()
            .compareTo(totalAdd)
        == 0);
    assert (this.lockableERC20
            .allowanceMin(this.owner, this.otherAccount)
            .send()
            .compareTo(BigInteger.ZERO)
        == 0);
    ...
    try {
      ...
      throw new Error(
          "Unexpectedly no revert when decreaseAllowance called that could make allowance negative");
    } catch (TransactionException ex) {
      System.err.println(
          " Revert Reason: "
              + RevertReason.decodeRevertReason(
    ...
    assert (this.lockableERC20.allowance(this.owner, this.otherAccount).send().compareTo(totalAdd)
        == 0);
    assert (this.lockableERC20
            .allowanceProvisional(this.owner, this.otherAccount)
            .send()
    ...
    // Any decrease will fail at the moment as more than parallelization factor number of
    ...
    assert (this.lockableERC20.allowance(this.owner, this.otherAccount).send().compareTo(start)
        == 0);
    assert (this.lockableERC20
            .allowanceProvisional(this.owner, this.otherAccount)
            .send()
            .compareTo(start.add(totalAdd).subtract(totalDecrease))
        == 0);
    assert (this.lockableERC20
            .allowanceMax(this.owner, this.otherAccount)
            .send()
            .compareTo(start.add(totalAdd))
        == 0);
    assert (this.lockableERC20
            .allowanceMin(this.owner, this.otherAccount)
            .send()
            .compareTo(start.subtract(totalDecrease))
        == 0);
  }
  ...
  public void transferFromNotLocked() throws Exception {
    ...
    assert (this.lockableERC20.allowance(this.otherAccount, this.owner).send().compareTo(amount2)
        == 0);
    try {
    ...
    assert (this.lockableERC20.balanceOfProvisional(this.otherAccount).send().compareTo(balOther)
        == 0);
  }
  ...
  public void transferFromLocked() throws Exception {
    ...
    // Unlock the contracts and apply the updates.
    this.lockableERC20.finalise(true, crosschainRootTxId).send();
    // The contract will still detect a crosschain transaction because the cbc is still indicating
    // this.
    ...
    assert (this.lockableERC20
            .balanceOf(this.owner)
            .send()
            .compareTo(INITIAL_SUPPLY_BIG.subtract(amount1))
        == 0);
    assert (this.lockableERC20
            .balanceOfMin(this.owner)
            .send()
            .compareTo(INITIAL_SUPPLY_BIG.subtract(amount1))
        == 0);
    assert (this.lockableERC20.balanceOfProvisional(this.owner).send().compareTo(balOwner) == 0);
