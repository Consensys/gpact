...
contract SignedEventStore is CrosschainVerifier, BytesUtil {
    ...
    function relayEvent(
        ...
        for (uint256 i = 0; i < len; i++) {
            require(signedEvents[signedEventDigest].whoVotedMap[signers[i]] == false, "Relayer has already signed event");
        ...
        if (signedEvents[signedEventDigest].whoVoted.length >= threshold) {
            ...
            functionCall.crossCallHandler(_blockchainId, _cbcAddress, _encodedEvent, _signature);
