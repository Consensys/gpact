...
/**
 * Manages the interaction between the application library and attestors for a certain
 * blockchain. The class asks an attestor to give the threshold signed event data. The attestor
 * should have cooperated with other attestors already to threshold sign the event data.
 *
 * TODO the Attestor service has not been implemented yet. As such, this class simulates calling
...
public class AttestorSigner implements MessagingVerificationInterface {
  ...
  public SignedEvent getSignedEvent(
    ...
    byte[] encodedEventInformation = abiEncodePackedEvent(this.bcId, contractAddress, eventFunctionSignature, eventData);
...
//    List<BigInteger> sigV = new ArrayList<>();
    ...
    return new SignedEvent(this.bcId, contractAddress, eventFunctionSignature,
      eventData, encodedSignatures);
