...
import "../attestor-sign/AttestorSignRegistrar.sol";
...
contract CrosschainVerifierTxRoot is CrosschainVerifier, Receipts {
    ...
    constructor(address _txReceiptRootStorage) {
        txReceiptRootStorage = TxReceiptsRootStorageInterface(
            _txReceiptRootStorage
        );
    }
    ...
    function decodeAndVerifyEvent(
        ...
        require(
            _expectedBlockchainId == eventProof.blockchainId,
            "Event not emitted by expected blockchain"
        );
        ...
        RLP.RLPItem[] memory keyAndReceipt = RLP.toList(
            RLP.toRLPItem(eventProof.encodedTxReceipt)
        );
        bytes memory receiptBytes = RLP.toData(keyAndReceipt[1]);

        bytes memory eventData;
        // If the CBC Contract is incorrect, extractEvent will fail.
        (, eventData) = extractEvent(
            eventProof.cbcContract,
            _eventSig,
            receiptBytes
        );

        bytes memory calculatedEncoded = abi.encodePacked(
            _expectedBlockchainId,
            eventProof.cbcContract,
            _eventSig,
            eventData
        );
    ...
    {
        RLP.RLPItem[] memory encodedEventProof = RLP.toList(
            RLP.toRLPItem(_encodedEventProof)
        );
        uint256 blockchainId = BytesUtil.bytesToUint256(
            RLP.toData(encodedEventProof[0]),
            0
        );
        address cbcContract = BytesUtil.bytesToAddress(
            RLP.toData(encodedEventProof[1])
        );
        bytes32 txReceiptRoot = BytesUtil.bytesToBytes32(
            RLP.toData(encodedEventProof[2]),
            0
        );
        ...
        require(
            proofOffsetsRlp.length == proofRlp.length,
            "Length of proofOffsets does not match length proof"
        );
        ...
        for (uint256 i = 0; i < proofRlp.length; i++) {
            proofOffsets[i] = BytesUtil.bytesToUint256(
                RLP.toData(proofOffsetsRlp[i]),
                0
            );
