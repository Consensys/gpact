...
public class LockableStorageTypesTest extends AbstractLockableStorageTest {
  ...
  public void bool() throws Exception {
    ...
    try {
      this.lockableStorageContract.test_setBool(keyTestBool, true).send();
  ...
  public void uint256() throws Exception {
    ...
    // Set the first value
    try {
      this.lockableStorageContract.test_setUint256(uint256A, val1).send();
    } catch (TransactionException ex) {
      if (ex.getTransactionReceipt().isPresent()) {
        TransactionReceipt receipt = ex.getTransactionReceipt().get();
        System.out.println("Revert Reason: " + receipt.getRevertReason());
    ...
    try {
      this.lockableStorageContract.test_setUint256(uint256B, val2).send();
    } catch (TransactionException ex) {
      if (ex.getTransactionReceipt().isPresent()) {
        TransactionReceipt receipt = ex.getTransactionReceipt().get();
        System.out.println("Revert Reason: " + receipt.getRevertReason());
    ...
    try {
      this.lockableStorageContract.test_setUint256(uint256A, val3).send();
    } catch (TransactionException ex) {
      if (ex.getTransactionReceipt().isPresent()) {
        TransactionReceipt receipt = ex.getTransactionReceipt().get();
        System.out.println("Revert Reason: " + receipt.getRevertReason());
  ...
  public void array() throws Exception {
    ...
    try {
      this.lockableStorageContract.test_getArrayValue(arrayA, BigInteger.TEN).send();
  ...
  public void map() throws Exception {
    ...
    this.lockableStorageContract.test_setMapValue(mapA, key1, val1).send();
    this.lockableStorageContract.test_setMapValue(mapA, key2, val2).send();
