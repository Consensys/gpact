...
public class CrosschainExecutor {
  ...
  private final Map<BlockchainId, List<SignedEvent>> signedSegmentEventsWithLockedContracts = new ConcurrentHashMap<>();
  ...
  public void startCall() throws Exception {
    CrossControlManager rootCbcContract = this.crossControlManagerGroup.getCbcContract(this.rootBcId);
    MessagingVerificationInterface messaging = this.crossControlManagerGroup.getMessageVerification(this.rootBcId);
    ...
    this.signedStartEvent = messaging.getSignedEvent(
  ...
  public void segment(BlockchainId blockchainId, BlockchainId callerBlockchainId, List<BigInteger> callPath) throws Exception {
    if (callPath.size() == 0) {
    ...
    CrossControlManager segmentCbcContract = this.crossControlManagerGroup.getCbcContract(blockchainId);
    MessagingVerificationInterface messaging = this.crossControlManagerGroup.getMessageVerification(blockchainId);
    ...
    SignedEvent signedSegEvent = messaging.getSignedEvent(
            this.crossControlManagerGroup.getAllBlockchainIds(),
            txr,
            segEventData,
  ...
  public void root() throws Exception {
    ...
    this.signedRootEvent = messaging.getSignedEvent(
            this.crossControlManagerGroup.getAllBlockchainIds(),
            txr,
            rootEventData,
            rootCbcContract.getCbcContractAddress(),
